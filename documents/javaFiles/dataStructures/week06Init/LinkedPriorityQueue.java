...
public class LinkedPriorityQueue<T extends Comparable>
   ...
   public void add(T element) {
	 ...
	 count++;
	 if(front == null || element.compareTo(front.getElement())<=0){
		 node.setNext(front);
		 front  = node;
	 }
	 else{
		 LinearNode<T> next = front;
		 while(next.getNext()!= null && element.compareTo(next.getNext().getElement()) > 0 ){
			 next = next.getNext();
		 }
   ...
   public T removeMin() {
	   if (front == null){
		   throw new NoSuchElementException("remove from an empty PQ");
	   }
