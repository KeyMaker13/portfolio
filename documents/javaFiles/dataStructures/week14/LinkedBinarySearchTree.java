...
public class LinkedBinarySearchTree<T extends Comparable<T>> extends
   ...
   public T removeMin() throws EmptyCollectionException {
      if (isEmpty()) // emtpy tree -- no min
         throw new NoSuchElementException("Empty tree -- can't remove min");
      count--;
      if (root.getLeft() == null) { // no left subtree -- right child is now root
      ...
      while (current.getLeft() != null) { // make current min node
         parent = current;
         current = current.getLeft();
      }
   ...
   public T remove2(T targetElement) {
      ...
      while (current != null && !found) {
         if (targetElement.equals(current.getElement())) {
            ...
            T min = findMin(current.getRight());
            if (min == null) {
               if (current == root) {
   ...
   public T removeMax() throws EmptyCollectionException {
      if (isEmpty()) // emtpy tree -- no min
         throw new NoSuchElementException("Empty tree -- can't remove max");
      count--;
      if (root.getRight() == null) { // no right subtree -- left child is now root
         T result = root.getElement();
         root = root.getLeft();
         return result;
      }
      ...
      while (current.getRight() != null) { // make current min node
         parent = current;
         current = current.getRight();
      }
      parent.setRight(current.getLeft());
