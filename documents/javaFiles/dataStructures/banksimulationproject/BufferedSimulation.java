...
public class BufferedSimulation {
	...
	private QueueADT<Customer> waitingLine; 

	public BufferedSimulation(String simname) {
		...
		currentTime = 0;
		name = simname;
		waitingLine = new LinkedQueue<Customer>();
	}
	
	public String	testingWaitingLine(){
		String r="";
		int i = 0;
		while (i<waitingLine.size()){
			r += "\n" +waitingLine.dequeue().toString();
		}
		return  r;
	}
	...
	private void handleEvent(Event event)
	{
		...
		{
			// when customer arrives either clerk is available or not
			
			// if clerk is available then service the customer
			if(findAvailableClerk() != null)
			{
			...
			// if no clerk is available then enqueue the customer in the waitingLine
			else
			{
				waitingLine.enqueue((Customer) event.getTarget());
			}
		}
		else if (event.getType() == Event.SERVICE_FINISHED)
		{
			...
			{
				clerk.assignCustomer(waitingLine.dequeue());
				eventQueue.add(clerk.getFinishEvent(currentTime));
			}
		}
	}
