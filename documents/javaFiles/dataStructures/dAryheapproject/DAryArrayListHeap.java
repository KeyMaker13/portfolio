...
public class DAryArrayListHeap <T extends Comparable>implements HeapADT<T>{
	private ArrayList<T> heap;
	int d;

	public DAryArrayListHeap(int dimension) {
	d=dimension;
	...
	*/
	...
	private int parent(int node) { // O(1) runtime	
		return (node - 1)/d;  // integer division rounds down
	}
	...
	private int childIndex(int current,int whichChild){
		
		return d*current+whichChild;
