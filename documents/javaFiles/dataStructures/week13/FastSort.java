...
public class FastSort <T extends Comparable<T>>{
  public void mergeSort(T[] data, int min, int max) {
    ...
    printArray(data, mid+1, max, "After sorting from " + (mid+1) + " to " + max);
...
//    printArray(data, min, max, "After merging from " + min + " to " + max);
  ...
  public void merge(T[] data, int min, int mid, int max) {
    ...
    int left = 0; // start of first sorted subarray
    int right = mid - min + 1; // start of second sorted subarray
  ...
  public void heapsort(T[] data, int min, int max) {
    HeapADT<T> heap = new ArrayListHeap<T>();
